...
function MobileSideBar({ collapsed, setCollapsed }: SidebarProps) {
  ...
  return (
    <>
      <Sider
        trigger={null}
        width={320}
        className={`fixed top-0 left-0 h-full block lg:hidden side-control overflow-y-auto ${collapsed ? `bg-transparent` : `bg-white`} ${collapsed ? 'hidden' : 'block'}`}
        style={{ position: 'fixed', left: 0, bottom: 0, zIndex: 1000 }}
      >
        {/* Close Button */}
        <div className="flex justify-end p-4">
          <Button
            ...
            onClick={() => setCollapsed(true)} // Close sidebar
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
        ...
        <div className="mt-5 mb-5" onClick={() => router.push('/dashboard')}>
          {collapsed ? null : (
            <Button
